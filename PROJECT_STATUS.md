...
- E11: `unit_verified`；geometry VERIFIED；live #1 `portal_activation_not_observed`；Axis.X PortalSize PASS on snapshot；callback UNPROVEN；`integration_verified`: NO。
...
- E11: `unit_verified`；runtime geometry deployed / real verified YES；live #1 attempted YES (`portal_activation_not_observed`)；root cause UNPROVEN pending diagnostic runtime；`integration_verified`: NO；
- E12: NOT STARTED；P1 Hard Gate: NOT PASSED；P2: NOT STARTED；
...
E10 contract/offline runtime/MineRL bridge is `unit_verified`. Controlled Mission geometry is VERIFIED. One real E10 conversion (`p1-e10-live-001`) produced `obsidian_conversion_ok` from lava source + one `use_item(water_bucket)` to water source + obsidian, `truth_missing_count=0`. Compact evidence: `runs/history/p1-e10-live-20260816-001/`. `integration_verified`: NO. E11 geometry real verified: YES. Live #1 (`p1-e11-live-001`) `portal_activation_not_observed`: fire at ignition, 0/6 portal. Offline PortalSize replica: Axis.X PASS on that snapshot; callback/tag/dimension-key UNPROVEN. Next exact task: authorize E11 diagnostic runtime instrumentation (`patches/minerl/e11-portal-activation-diagnostic.patch`). Do not retry, Gradle, or start E12. P1 Hard Gate: NOT PASSED. P2: NOT STARTED. Attempt-014 remains historically unexplained.
