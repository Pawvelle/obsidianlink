...
- E11: `unit_verified`；geometry VERIFIED；live #1 `portal_activation_not_observed`；diag-002 Render-thread write PROVEN；live #2/#3 marshal timeout (`tested_action_count=0`)；real reviewed success: NO；`integration_verified`: NO。
...
- E11: `unit_verified`；runtime geometry YES；live #1 `portal_activation_not_observed`；live #2/#3 marshal timeout (`truth_identity_mismatch`)；real reviewed success: NO；`integration_verified`: NO；
- E12: NOT STARTED；P1 Hard Gate: NOT PASSED；P2: NOT STARTED；
