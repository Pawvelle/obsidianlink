...
Next: first real blocker remains E8 before-truth `truth_block_unknown`. Existing Pilot #3 evidence cannot identify the raw Minecraft block or probe cell. Evaluator-only E8 diagnostics now record unknown raw block names, probe world/grid cells, grid anchor, anchor source, position, dimension, and portal-grid payload presence; `other` still fail-closes and the block whitelist is unchanged. Historical pilots `20260817-001`, `20260817-002`, and `20260817-003` remain failed evidence and are not rewritten. A new authorized Full Suite Pilot #4 may be requested after reviewing those diagnostics. Do not start P2. P1 Hard Gate: NOT PASSED. P2: NOT STARTED.
