...
E0/E1/E2 are not promoted to `integration_verified`. Existing policy: live runtimes and `p1_validation_manifest()` / `--check` are fail-closed offline surfaces and never emit that claim; AGENTS.md forbids declaring `integration_verified` before the P1 Hard Gate; a single reviewed success is recorded evidence, not suite promotion. `benchmark_evaluated` is not claimed.
...
P1 E2 inventory observation is complete through one reviewed authorized real MineRL calibration success. Stop here for this development session. E3 selected-item observation remains the next P1 validation item but has not started. P1 Hard Gate has not passed and P2 must not begin.
