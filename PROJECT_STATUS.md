...
- E11: `unit_verified`；geometry VERIFIED；completion-barrier runtime SHA-256 `6b5705e4…` semantic diff clean. Only `EnvServer`、`ReplaySender`、`ServerPlayNetHandler` and their compiler line-number artifacts changed; PortalSize、FlintAndSteelItem、Entity、ServerPlayerEntity、audio、E12/evaluator/geometry code unchanged. `p1-e11-completion-barrier-20260817-004` used one fresh process / one reset / zero retry / `tested_action_count=1`; the server completion acknowledgement released the barrier after normal vanilla interaction. Before: 14/14 obsidian, 6/6 interior air, portal=0, `truth_missing_count=0`; after: portal=6/6, `truth_missing_count=0`, outcome `portal_activation_ok`. Root cause fixed: action-delivery/evaluator-ordering race. Real reviewed success: YES；`integration_verified`: NO。
