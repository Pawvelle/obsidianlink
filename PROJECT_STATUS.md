...
P1 startup reliability calibration completed 20 fresh-process attempts: 18 success / 2 infrastructure failures (90% first-attempt success). Attempt-006 is a confirmed `liblwjgl_stb` / `Sound engine` SIGSEGV; attempt-014 remains a separate unresolved mission/reset reply failure. The narrowly scoped `disable-client-audio.patch` has now been rebuilt from a reproduced P1 deployed-runtime baseline and deployed to the fixed `mc-agent` MineRL runtime. One authorized fresh-process smoke with `max_reset_attempts=1` completed `reset`, produced an initial observation, and returned from `close`; no validation action, native crash, Malmo EOF, timeout, or tracked residual descendant was observed. This single smoke does not establish reliability or promote any verification state; `process_release_proven=false` remains.

Do not start E10. P1 Hard Gate: NOT PASSED. P2: NOT STARTED. The next task is an independently authorized 20-episode fresh-process startup reliability validation with `max_reset_attempts=1`, followed by evidence review. The current `vendor/minerl/minerl/MCP-Reborn` build tree still contains additional world-decorator and portal-transition code absent from the deployed P1 runtime and must not be deployed as-is. Attempt-014 remains independently unresolved unless the enhanced evidence captures a recurrence.
