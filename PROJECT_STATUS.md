...
- E11: `unit_verified`；geometry VERIFIED；live #1 `portal_activation_not_observed`；diag-002 Render-thread write PROVEN；live #2–#4 marshal timeout (`tested_action_count=0`)；real reviewed success: NO；`integration_verified`: NO。
...
- E11: `unit_verified`；runtime geometry YES；live #1 `portal_activation_not_observed`；live #2–#4 marshal timeout (`truth_identity_mismatch`)；real reviewed success: NO；`integration_verified`: NO；
- E12: NOT STARTED；P1 Hard Gate: NOT PASSED；P2: NOT STARTED；
...
E10 real conversion reviewed success: YES (`p1-e10-live-001`). `integration_verified`: NO. E11 geometry YES. Live #1 `portal_activation_not_observed`. Live #2 waited before `addAction`. Live #3 queued then timed out before `waitForNextObservation`. Live #4 (`p1-e11-live-004`) awaited after `waitForNextObservation` (`stepClient:772`); `processRightClickBlock` still never ran; `tested_action_count=0`. Compact: `runs/history/p1-e11-live-20260817-003/`. The broad paused-executor patch (`8b2258e`) is rejected because it admits all queued tasks; its narrowed E11-only follow-up preserves ordinary `super.canRun` semantics offline. Reapplying frozen audio mitigation restored byte-identical `SoundEngine`, but the final JAR still differs in unrelated `Entity` / `ServerPlayerEntity` classes, so it is not approved for deployment or a fresh E11 run. E11 real reviewed success: NO. Next exact task: reconstruct a byte-clean frozen baseline before considering E11. Do not start E12. P1 Hard Gate: NOT PASSED. P2: NOT STARTED. Attempt-014 remains historically unexplained.
