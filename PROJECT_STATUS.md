...
All E0–E9 contracts, offline runtimes, and MineRL adapters are `unit_verified`; each has at least one reviewed real success, but none is `integration_verified` and `process_release_proven=false` remains. E10 contract/offline runtime/MineRL bridge is `unit_verified`; E10 geometry real verified: YES; E10 real conversion reviewed success: YES; `integration_verified`: NO. E11 contract/offline runtime/MineRL bridge is `unit_verified`; E11 runtime geometry deployed / real verified: YES. A clean canonical runtime excluded old marshal/paused-executor/diagnostic/E12 code and kept the integrated server unpaused in environment mode. Its single real E11 calibration (`p1-e11-canonical-runtime-20260817-002`) still produced fire + 0/6 portal, outcome `portal_activation_not_observed`; real reviewed success: NO; `integration_verified`: NO.
...
- E11: `unit_verified`；geometry VERIFIED；canonical runtime semantic diff clean（only `IntegratedServer.class` + metadata；Entity/ServerPlayerEntity unchanged；no E12）；canonical live: one fresh process / one reset / zero retry / `tested_action_count=1`，before 14/14 obsidian + 6/6 air + 0 portal，after fire + 0/6 portal，`truth_missing_count=0`，outcome `portal_activation_not_observed`；real reviewed success: NO；`integration_verified`: NO。
...
- E11: `unit_verified`；runtime geometry YES；canonical live `p1-e11-canonical-runtime-20260817-002` = `portal_activation_not_observed`，0/6 portal，`tested_action_count=1`，retry=0；real reviewed success: NO；`integration_verified`: NO；
- E12: NOT STARTED；P1 Hard Gate: NOT PASSED；P2: NOT STARTED；
...
E10 real conversion reviewed success: YES (`p1-e10-live-001`). `integration_verified`: NO. A new canonical P1 runtime was rebuilt from the frozen MCP-Reborn source with only `obsidianlink-envserver`, canonical audio source, E10 geometry, E11 obsidian geometry, and environment-mode integrated-server unpause changes. `shadowJar` SHA-256 is `684c20ec…`; semantic diff against the prior production JAR changes only `IntegratedServer.class` and `version.properties`. EnvServer, SoundEngine, PortalSize, FlintAndSteelItem, Entity, and ServerPlayerEntity are byte-identical; old marshal/paused-executor/diagnostic/E12 markers are absent. One authorized real run (`p1-e11-canonical-runtime-20260817-002`) used one fresh process, one reset, one accepted `use_item(flint_and_steel)`, and zero retry. Before: 14/14 obsidian, 6/6 air, 0 portal, overworld, `truth_missing_count=0`. After: fire at ignition, 0/6 portal, `truth_missing_count=0`; outcome `portal_activation_not_observed`. No paused-server log occurred. The unique current blocker is now the unproven server-side packet/`FlintAndSteelItem` execution after normal ReplaySender client interaction. Do not retry automatically and do not start E12. P1 Hard Gate: NOT PASSED. P2: NOT STARTED.
