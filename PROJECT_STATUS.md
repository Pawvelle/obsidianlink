...
Pilot #4 的 suite 字段为 `verdict=hard_gate_success`、`p1_hard_gate_passed=true`。这只证明当前冻结版本获得一次真实完整 P1 suite 成功运行，不把 E0–E12 提升为 `integration_verified`，也不关闭进入 P2 所需的重复性要求。进入 P2 前必须完成真实环境 validation suite 的稳定重复成功、`truth_missing=0`、无人工干预。规划建议至少 20 个 fresh episodes。P2: NOT STARTED。每次真实 MineRL/Minecraft 运行与每次 Gradle 构建仍需用户单独授权。
...
当前完整 P1 suite 已获得一次真实完整成功运行。Next: 按现有重复性要求判断是否满足离开 P1 的稳定性 Hard Gate；需要另一次明确授权才会再跑 suite。Do not start P2. Do not set `integration_verified`. P2: NOT STARTED.
