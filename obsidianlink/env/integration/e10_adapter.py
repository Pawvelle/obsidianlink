"""Narrow MineRL bridge for the P1 E10 vanilla obsidian-conversion calibration.

Importing this module does not import MineRL or construct a production
backend. Conversion truth comes only from the backend-retained evaluator
...
class MineRLE10ObsidianAdapter(MineRLE0LifecycleAdapter):
    """Own one backend and expose exactly the action/truth E10 requires."""
    ...
    def _build_compatibility_task(episode_id: str) -> object:
        return build_e10_compatibility_task(episode_id)
    ...
    def server_truth_snapshot(self) -> ServerTruthSnapshot | None:
        ...
        value = getter(E10_PROBE_WORLD_CELLS)
        return None if value is None else server_truth_snapshot(
            value, expected_cells=E10_PROBE_WORLD_CELLS
        )
    ...
    def execute_conversion_stimulus(self, action: MacroAction) -> ObsidianConversionActionExecution:
        if not isinstance(action, MacroAction) or action.action_type != E10_STIMULUS_ACTION_TYPE:
            raise ValueError("E10 stimulus must be MacroAction('use_item')")
        if (
            action.target != E10_STIMULUS_ITEM_NAME
            or action.duration_ticks != E10_DURATION_TICKS
            or dict(action.parameters)
        ):
            raise ValueError("E10 stimulus differs from frozen calibration")
        self._tested_action_count += 1
        if self._tested_action_count != 1:
            raise RuntimeError("E10 permits exactly one stimulus action")
        backend = self._ensure_backend()
        step = getattr(backend, "step", None)
        if not callable(step):
            raise RuntimeError("MineRL backend step is not callable")
        result = step({E10_AGENT_ID: action})
        ...
        if leaked:
            raise ValueError("E10 backend info leaked evaluator truth: " + ", ".join(leaked))
    ...
    def observe_wait(self) -> None:
        ...
        if self._tested_action_count != 1:
            raise RuntimeError("E10 observation waits require the stimulus to have run")
        backend = self._ensure_backend()
        step = getattr(backend, "step", None)
        if not callable(step):
            raise RuntimeError("MineRL backend step is not callable")
        result = step({E10_AGENT_ID: MacroAction.wait()})
        if not isinstance(result, BackendStep):
            raise TypeError("MineRL backend step must return BackendStep")
        leaked = sorted(key for key in result.info if key in EVALUATOR_TRUTH_LEAK_KEYS)
        if leaked:
            raise ValueError("E10 wait info leaked evaluator truth: " + ", ".join(leaked))
    ...
    ) -> Callable[[], "MineRLE10ObsidianAdapter"]:
        def factory() -> MineRLE10ObsidianAdapter:
            return cls(
