"""D1-02 Water Presence — live 2B / 4B evaluation.

Same 640×360 / max_steps=1 / hidden-GT contract as D1-01 Lava.
Prompt, evaluator, and model weights are not tuned.

Saved to ``obsidianlink/experiments/runs/d1_02_water_<model>_<N>ep_<TS>.json``.
...
::
    ...
    PYTHONPATH=/Users/joey/Documents/Projects/ObsidianLink \\
        ...
        obsidianlink/experiments/run_d1_02.py \\
...
def _run_one_episode(
    ...
    task = D1_02_WATER_TASKS[condition]
    env_id = D1_02_WATER_ENV_IDS[condition]
    print(
    ...
    env = ControlledSceneEnv(
        ...
        setup_actions=d1_02_setup_actions(condition),
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        default="both",
        help="Which D1-02 scene(s) to run. Default: positive then negative.",
    )
    parser.add_argument(
        "--save-dir",
        default=_RUNS_DIR,
    ...
    run_stem = f"d1_02_water_{model_name}_{args.num_episodes}ep_{timestamp}"
    frames_root = os.path.join(args.save_dir, run_stem + "_frames")

    print("ObsidianLink — D1-02 Water Presence live evaluation")
    print("  task_id     : d1_02_water_presence")
    print(f"  resolution  : {D1_V2_RESOLUTION[0]}x{D1_V2_RESOLUTION[1]}")
    print(f"  conditions  : {', '.join(conditions)}")
    print(f"  model       : {args.model_path}")
    print(f"  model_name  : {model_name}")
    print(f"  N / cond    : {args.num_episodes}")
    print(f"  warmup      : {D1_02_WARMUP_STEPS}")
    print(f"  device      : {args.device}")
    print("  prompt      : D1PresenceAgent(target='water') (unchanged)")
    ...
    n = agg["n_episodes"]
    print(f"\n=== Aggregate ({model_name}, D1-02, {n} episodes) ===")
    print(
    ...
    payload = {
        ...
        "env_ids": {c: D1_02_WATER_ENV_IDS[c] for c in conditions},
        "warmup_steps": D1_02_WARMUP_STEPS,
