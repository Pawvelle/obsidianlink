"""Orchestrator helper for the R6 Casting-S-C3 frame driver.

This module is the *only* place in the R6 C3 frame driver family that
is allowed to read a :class:`TaskInstance` object's
``scenario_parameters``. It builds the strictly-validated, immutable
:class:`PublicC3FrameDriverContext` that the driver function itself
consumes. The driver function in
:mod:`obsidianlink.drivers.casting_s_c3_frame` never sees the
...
and the task's public budgets / initial inventories.
"""
...
def build_public_c3_frame_driver_context_from_task(
    task: TaskInstance,
) -> PublicC3FrameDriverContext:
    """Build a :class:`PublicC3FrameDriverContext` from a task instance.

    This helper is the *only* function in the R6 C3 frame driver
    ...
    via :func:`run_casting_s_c3_frame_driver`.
    """
    if not isinstance(task, TaskInstance):
        raise ValueError("task must be a TaskInstance")
    if task.workflow != WORKFLOW_C3_FRAME:
        raise ValueError(
            f"task.workflow must be {WORKFLOW_C3_FRAME!r}, got "
            f"{task.workflow!r}"
        )
    ...
    if not isinstance(scenario, Mapping) or not scenario:
        raise ValueError(
            "task.scenario_parameters is required to build the C3 frame "
            "driver context"
        )
...
__all__ = ["build_public_c3_frame_driver_context_from_task"]
