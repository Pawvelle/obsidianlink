"""Minimal named-item building skill."""
...
def equip_item(controller: MinecraftController, item: str) -> tuple[bool, int]:
    start = controller.steps
    if controller.observe().selected_item == item:
        return True, 0
    if controller.exhausted:
        return False, 0
    observation = controller.step(Action(ActionType.EQUIP, target=item))
    return observation.selected_item == item, controller.steps - start


class BuildStructureSkill:
    name = "build_structure"
    description = "Equip an inventory block and place a small bounded number of it."
    ...
    def execute(
        ...
        selected, _ = equip_item(controller, item)
        if not selected:
        ...
        for index in range(count):
            if controller.exhausted:
                break
            controller.step(Action(ActionType.PLACE, target=item, sneak=True))
...
__all__ = ["BuildStructureSkill", "equip_item"]
