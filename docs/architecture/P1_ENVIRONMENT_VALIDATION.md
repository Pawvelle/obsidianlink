...
E0 contract 已完成；E0 离线 runtime 为 `unit_verified`；E0 MineRL integration bridge 已实现并离线测试。真实 MineRL E0 尚未执行，E0 不是 `integration_verified`。`p1_validation_manifest()` 仍将 E0–E12 标为 `not_run`。E1–E12 尚未实现。未来真实 E0 入口为 `python -m obsidianlink.env.integration.e0_run`，必须显式提供 `--execution-mode authorized_live_e0 --authorized-live-run e0_reset_close`。每次真实 MineRL/Minecraft 与每次 Gradle 构建需用户单独授权。
