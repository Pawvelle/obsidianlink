...
E0–E12 contract / adapter / offline runtime / live bridge 为 `unit_verified`，且各有 reviewed real success；均不是 `integration_verified`，object-level `process_release_proven=false`，`p1_validation_manifest()` 仍将 E0–E12 标为 `not_run`。E11 runtime geometry deployed / real verified = **YES**；real E11 reviewed success = **YES** (`p1-e11-completion-barrier-20260817-004`, `portal_activation_ok`, `tested_action_count=1`, retry=0, portal=6/6, `truth_missing_count=0`)；`integration_verified` = **NO**。E12 real reviewed success = **YES** (`p1-e12-dimension-transition-20260817-001`, `dimension_transition_ok`, before=`minecraft:overworld`, after=`minecraft:the_nether`, `tested_action_count=1`, retry=0, `truth_missing_count=0`)；`integration_verified` = **NO**。一次 E12 成功不是离开 P1 的稳定性 Hard Gate。E8/E9/E10/E11/E12 evaluator-only truth 与 Agent-visible observation 保持隔离。当前完整 P1 suite 已获得一次真实完整成功运行（Pilot #4）。Stability campaign `p1-stability-20260817` 为 0/20。P2 Roadmap 在 step 1（offline kernel freeze）已部分开始（commit `a5131dc`）；P2 Benchmark Kernel v2 现已 offline FROZEN（freeze audit 2026-08-18）。该步骤不依赖 P1 hard gate，也未把任何 E0–E12 标为 `integration_verified`。完整 P2 推进仍以 P1 重复性要求为准。

当前完整 P1 suite 已获得一次真实完整成功运行：`p1-e0-e12-suite-20260817-004`。Stability campaign `p1-stability-20260817` 另产生 20 个新的 fresh attempts，0/20 完整成功，全部在 E0 `reset_failed`。历史失败证据保留：`p1-e0-e12-suite-20260817-001` 在 E1 因 OS `process_release_not_proven` 停止；第二次授权 live pilot `p1-e0-e12-suite-20260817-002` 在 E4 因 `truth_missing` 停止（E4 `camera_ok` 但未写出 `truth_missing_count`）；第三次授权 live pilot `p1-e0-e12-suite-20260817-003` 在 E8 因 `truth_missing` 停止（E8 `truth_block_unknown`，`truth_missing_count` 为空）。授权 targeted E8 diagnostic `e8-live-20260817-001` 得到 `block_truth_ok`，Pilot #4 的 E8 同样 `block_truth_ok`。每次真实运行与每次 Gradle 构建仍需用户单独明确授权。

进入 P2 完整 task instance / Multi-Agent / L1–L4 推进前，仍要求完整 suite 稳定重复成功、`truth_missing=0`、无人工干预。P2 step 1 (offline kernel freeze) 单独完成，不要求 P1 hard gate 通过，也不把 E0–E12 设为 `integration_verified`。仓库尚未冻结数值通过阈值（`repeatability_threshold_not_frozen`）。建议至少 20 个 fresh episodes，最终次数、seed、timeout 与失败处理后续冻结。
...
Third authorized live pilot `p1-e0-e12-suite-20260817-003` executed E0–E8 and stopped. Canonical `684c20ec…` was verified for all ten completed steps; Gradle was not invoked. E0–E7 (including E7 water and E7 lava) succeeded with OS `process_release_proven=true` and no residual PIDs. E4–E7 used case-specific evaluator outcomes rather than `truth_missing_count`. E8: `real_execution_performed=true`, OS `process_release_proven=true`, scientific outcome `truth_block_unknown` (`ValueError: unknown block truth`) before stimulus (`tested_action_count=0`); the live record has `truth_missing_count=null`, so the suite verdict is `truth_missing`. `p1_hard_gate_passed=false`; no retry; E9–E12 not launched. Evidence: `runs/p1_validation_suite/p1-e0-e12-suite-20260817-003/`. This does not set `integration_verified`. Historical pilots `20260817-001` and `20260817-002` remain failed evidence and are not rewritten. Later E8 evaluator-only diagnostics record unknown raw block names and probe cells on `truth_block_unknown`; that does not rewrite this pilot.

Authorized targeted E8 diagnostic `e8-live-20260817-001` (2026-08-17) used already-active canonical `684c20ec…` and did not invoke Gradle. Outcome `block_truth_ok`; before all three probes `air`; after target `dirt` with controls unchanged; `truth_missing_count=0`; `tested_action_count=1`; `anchor_source=expected_spawn_fallback`; `grid_anchor_world=(0, 4, 0)`; position `(0.5, 4.0, 0.5)`; dimension `minecraft:overworld`; `unknown_block_diagnostics=null`. Object-level cleanup `process_release_proven=false`; no residual Java/MineRL PIDs after close. Evidence: `runs/p1_e8_block_truth/e8-live-20260817-001/`. This does not rewrite Pilot #3 and does not set `integration_verified`. The Pilot #3 `truth_block_unknown` was not reproduced.

Fourth authorized live pilot `p1-e0-e12-suite-20260817-004` (2026-08-17) executed all 15 steps once, with no retry and no Gradle. Canonical `684c20ec…` was already active for E0–E10; E11 switched to completion-barrier `6b5705e4…`; E12 switched to portal-fixture `f459c36b…`. All scientific outcomes succeeded. E8: `block_truth_ok`, `tested_action_count=1`, before air/air/air, after dirt/air/air, `truth_missing_count=0`, `unknown_block_diagnostics=null`, `anchor_source=expected_spawn_fallback`. E8–E12 `truth_missing_count=0`. Every step OS `process_release_proven=true` with empty residual PIDs. `real_execution_performed=true`; `human_intervention=false`; `stopped_after=null`. Suite verdict `hard_gate_success`; this run’s `p1_hard_gate_passed=true`. `integration_verified=false`; `p1_validation_manifest()` remains `not_run`. Evidence: `runs/p1_validation_suite/p1-e0-e12-suite-20260817-004/`. Historical pilots `20260817-001`, `20260817-002`, and `20260817-003` remain failed evidence and are not rewritten. This records one real complete P1 suite success; repeatability required to leave P1 remains a separate judgment. P2 step 1 (offline kernel freeze) is now complete; full P2 task instance / Multi-Agent / L1–L4 progression is not started.

Authorized stability campaign `p1-stability-20260817` (2026-08-17) executed 20 new fresh full-suite attempts on frozen git `28583f8`, excluding Pilot #4. No retry, no Gradle, no production-code change. All 20 stopped after E0: outcome `reset_failed`, suite verdict `validation_failed`, identical error `MineRL reset failed after 1 attempts: a bytes-like object is required, not 'NoneType'`. Canonical `684c20ec…` verified each time. Each executed E0 had OS `process_release_proven=true` and empty residual PIDs; suite-level `process_release_proven` is false because the 15-step contract was incomplete. E1–E12 were not launched. Empirical success rate 0/20. Repeatability threshold is not frozen. Evidence: `runs/p1_stability_campaign/p1-stability-20260817/`. This does not set `integration_verified` and does not start P2.

## P2 step 1 — minimal Benchmark Kernel freeze (offline, no P1 hard gate)

P2 Roadmap phase kernel contract is **FROZEN** (freeze audit 2026-08-18). Remaining P2 work (D1–D6 / L1–L4 / Multi-Agent / real-backend attach) is not started. Step 1 is offline and does not depend on the P1 hard gate: no LLM, no real MineRL/Minecraft, no Gradle, no paid API. The freeze reuses every pre-existing v2 primitive in `obsidianlink/benchmark` (`TaskIdentity`, `Evaluator` / `EvaluatorVerdict`, `EvidenceRecord` + `EvidenceChannel` (`AGENT_VISIBLE` / `EVALUATOR_ONLY`), `MetricRecord`, `BenchmarkRunner` / `RunnerResult`, `VerificationLevel`) and `obsidianlink.core.types` (`Observation` / `MacroAction` / `BackendStep` via duck typing; the kernel module does not import `core.types` so the v2 architecture guard `obsidianlink.benchmark` ≠ `obsidianlink.core.types` stays intact).

- New: `obsidianlink.benchmark.run_record.BenchmarkRunRecord` (frozen aggregate dataclass: `task`, `runner_status`, `verdict`, `evidence`, `metrics`, `verification_level`, `schema_version = p2.benchmark.run_record.v1`).
- New: `obsidianlink.benchmark.run_record.run_benchmark(episode_id, task, backend, agent, evaluator, max_steps, initial_evaluator_state, metrics_hook)` — single function driver. Writes observation / action on `AGENT_VISIBLE`; runner_error / close_error / verdict on `EVALUATOR_ONLY`. Projects observations to a whitelisted agent-visible surface so evaluator-only data never reaches the agent.
- New: `obsidianlink.benchmark.run_record.write_run_record` / `load_run_record` — JSON round-trip helpers keyed by `schema_version`.
- New: `tests/test_p2_benchmark_kernel.py` — original 8 offline unit cases at commit `a5131dc`; freeze audit 2026-08-18 covers 11 cases (adds terminated/truncated stop, close-failure fail-closed, and verdict `agent_id` round-trip). All passing.
- No new abstract base class, registry, plugin system, dependency-injection container, or framework. `obsidianlink.benchmark` remains solver- and model-independent.

P1 historical evidence is preserved unchanged: Pilots #1–#4, Full Suite #5, `p1-stability-20260817` 0/20, `campaign-path-recheck-20260817-001` 15/15 single-campaign-path reproduction, and `p1-e0-e12-suite-20260817-005` independent recheck are all untouched in `runs/`. None is reclassified as `integration_verified`. None of the P1 E0–E12 contracts, runtime switching, process-release observer, evaluator truth rules, or hard-gate logic were modified. P1 hard gate is still open (stability repeatability not frozen). `p1_validation_manifest()` remains `not_run`. Original kernel commit `a5131dc`; terminated/truncated/close-failure `77c6c4e`; freeze audit SHA `84c6c23`.
