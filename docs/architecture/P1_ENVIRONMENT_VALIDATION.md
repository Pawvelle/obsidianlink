...
下一项经授权的 P1 工作是 **marshal portal placement onto the server thread**。不要为确认再跑一次，不要改 geometry/evaluator/window，不要开始 E12。每次真实运行仍需用户单独明确授权。
