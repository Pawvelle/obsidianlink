...
E0–E5 contract / adapter / offline runtime / live bridge 为 `unit_verified`。E0–E5 各有已审查的真实成功证据，但都不是 `integration_verified`：`process_release_proven` 仍为 false，live runtime / `--check` / `p1_validation_manifest()` 不得自动 promotion，P1 Hard Gate 要求完整 suite 稳定重复成功。E6 contract / adapter / offline runtime / live bridge 为 `unit_verified`，并有一次已审查的真实 `placement_ok`（`p1-e6-live-001`，world `(0, 4, 1)` / atSpawn grid `(0, 0, 1)`）；E6 不是 `integration_verified`。E7 contract / adapter / offline runtime / live bridge 为 `unit_verified`；WATER 有一次已审查的真实 `bucket_ok`（`p1-e7-water-live-001`，world `(0, 4, 1)` / atSpawn grid `(0, 0, 1)`）；LAVA 尚未真实运行；E7 不是 `integration_verified`。`p1_validation_manifest()` 仍将 E0–E12 标为 `not_run`。E8–E12 尚未实现。E0 入口为 `python -m obsidianlink.env.integration.e0_run`。E1 入口为 `python -m obsidianlink.env.integration.e1_run`。E6 入口为 `python -m obsidianlink.env.integration.e6_run`。E7 入口为 `python -m obsidianlink.env.integration.e7_run`。每次真实 MineRL/Minecraft 与每次 Gradle 构建需用户单独授权。一次 E7 live authorization 只对应一个 variant、一个 fresh episode、一次 `use_item`。
