...
进入 P2 前仍要求完整 suite 稳定重复成功、`truth_missing=0`、无人工干预。一次完整 suite success 不自动开始 P2，也不把 E0–E12 设为 `integration_verified`。仓库尚未冻结数值通过阈值（`repeatability_threshold_not_frozen`）。建议至少 20 个 fresh episodes，最终次数、seed、timeout 与失败处理后续冻结。
