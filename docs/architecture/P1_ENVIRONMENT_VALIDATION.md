...
P2 Roadmap phase is `IN PROGRESS` only at step 1 (kernel freeze). Step 1 is offline and does not depend on the P1 hard gate: no LLM, no real MineRL/Minecraft, no Gradle, no paid API. The freeze introduces the missing aggregate and a single driver, reusing every pre-existing v2 primitive in `obsidianlink/benchmark` (`TaskIdentity`, `Evaluator` / `EvaluatorVerdict`, `EvidenceRecord` + `EvidenceChannel` (`AGENT_VISIBLE` / `EVALUATOR_ONLY`), `MetricRecord`, `BenchmarkRunner` / `RunnerResult`, `VerificationLevel`) and `obsidianlink.core.types` (`Observation` / `MacroAction` / `BackendStep` via duck typing; the kernel module does not import `core.types` so the v2 architecture guard `obsidianlink.benchmark` ≠ `obsidianlink.core.types` stays intact).
...
- New: `tests/test_p2_benchmark_kernel.py` — 8 offline unit cases (full episode, channel separation, observation frame stripping, JSON round-trip, metrics hook, `TaskIdentity` validation, evaluator-only state passing, error path). All passing.
