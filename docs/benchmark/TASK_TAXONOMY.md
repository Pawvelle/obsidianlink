...
- `mode`: `single` / `multi`
- `level`: Diagnostic 使用 D1–D6；End-to-End 与其 Generalization/Recovery 变体使用 P1–P4
...
|---|---|---|
| P1 | Controlled Construction | attributed Nether entry |
| P2 | Resource Interaction | attributed Nether entry |
| P3 | Resource Acquisition | attributed Nether entry |
| P4 | Open-World Construction | attributed Nether entry |

level 间只通过初始条件、资源依赖、距离和环境变化增加难度。单块、多块、frame 或 ignition 是 milestone，不是 P-level。

## Generalization & Recovery

`generalization_recovery` 任务保留其基础 P1–P4 level，并添加冻结 variation/recovery profile。未来 profile 覆盖 seed、spawn、yaw、资源距离/分布、terrain、obstacles，以及 no-world-effect、placement failure、resource missing、path blocked、subgoal infeasible、state mismatch、casting error。
...
例如 `nether_portal_construction_s_end_to_end_p1_controlled`。Canonical name 不替代唯一 `task_instance_id`。

## Legacy IDs

`casting_c1_fixed`、`casting_c3_fixed`、`casting_s_c3_fixed`、`casting_s_c4_fixed`、`casting_s_c5_fixed` 及 Route A0 保留兼容 ID，但只属于 v1 legacy/calibration/regression。它们的 C1–C5 是历史 capability progression，不是 v2 P1–P4。
