...
唯一 active engineering task 是 `P1-REAL-MINERL-ENVIRONMENT-VALIDATION`。E0–E5 offline runtime 为 `unit_verified`，并各有已审查的真实成功证据（E5 第二次为首次 reviewed movement success）。E6 contract / offline runtime / MineRL adapter 为 `unit_verified`，并有一次已审查的真实 `placement_ok`（`p1-e6-live-001`）。E7 contract / offline runtime / MineRL adapter 为 `unit_verified`；WATER 有一次已审查的真实 `bucket_ok`（`p1-e7-water-live-001`）；LAVA 有一次已审查的真实 `bucket_ok`（`p1-e7-lava-live-001`）；E7 不是 `integration_verified`。E8 contract / ServerTruthSnapshot block portion / generalized target-region block truth / MineRL adapter 为 `unit_verified` / offline verified；attempt #1 `p1-e8-live-001` 在 reset 因 JVM SIGSEGV / Malmo EOF 失败（SAME_FINGERPRINT as E5 #1，不是 block-truth 能力失败）；attempt #2 `p1-e8-live-002` 为已审查的真实 `block_truth_ok`；E8 reviewed real success: YES，仍不是 `integration_verified`。E9 contract / generalized fluid truth / MineRL adapter / live bridge 为 `unit_verified` / offline tested；source/flowing 保持可区分。E9 WATER attempt #1 `p1-e9-water-live-001` 在 reset 因 JVM SIGSEGV / Malmo EOF 失败（SAME_FINGERPRINT as E5 #1 / E8 #1，不是 fluid-truth 能力失败）；没有 before/after fluid snapshots；`tested_action_count=0`。E9 WATER attempt #2 `p1-e9-water-live-002` 为已审查的真实 `fluid_truth_ok`：before 目标格 `air`/`none`，一次 `use_item(water_bucket)`，after 目标格 `observed_block=water` / `fluid_type=water` / `flow_state=source`（可与 `flowing_water` 区分），control 仍为 `air`，`truth_missing_count=0`。E9 LAVA attempt #1 `p1-e9-lava-live-001` 为已审查的真实 `fluid_truth_ok`：before 目标格 `air`/`none`，一次 `use_item(lava_bucket)`，after 目标格 `observed_block=lava` / `fluid_type=lava` / `flow_state=source`（可与 `flowing_lava` 区分），control 仍为 `air`，`truth_missing_count=0`。E9 WATER reviewed real success: YES。E9 LAVA reviewed real success: YES。E9 real calibration coverage: WATER + LAVA complete。E9 reviewed real success: YES。E9 不是 `integration_verified`。E0–E9 都不是 `integration_verified`（无自动 promotion，P1 Hard Gate 未通过，`process_release_proven=false`）。E10–E12 尚未开始。没有运行 Gradle，没有调用付费模型 API。P1 Hard Gate 未通过，P2 不得开始。E10 原版水熔岩生成黑曜石、E11 portal activation、E12 dimension transition 均未获得 `integration_verified`。

下一步不是自动重跑 E9，也不是开始 E10。需要单独授权一次独立的 E10 原版水熔岩 → 黑曜石真实 MineRL calibration。P1 的 hard gate 是稳定重复成功、`truth_missing=0`、无人工干预；建议至少 20 个 fresh episodes，最终次数在实验冻结阶段确定。
