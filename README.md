...
当前最小未验证点是 P1 Hard Gate 剩余要求：完整 suite 的稳定重复成功、`truth_missing=0`、无人工干预，以及未证明的 process release。E0–E12 全套件编排与 OS-level process-release 检查已落地为离线 `unit_verified` 合同，但一次授权 live pilot 尚未运行，因此 `process_release_proven=false`，Hard Gate 仍未通过。Canonical JAR 仍拒绝 `portal` DrawBlock；E12 fixture 只用于该授权校准。
