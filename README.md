...
当前最小未验证点是 P1 Hard Gate。授权 full-suite live pilot `p1-e0-e12-suite-20260817-001` 已运行一次：E0 `lifecycle_ok` 且 OS `process_release_proven=true`；E1 `rgb_ok` 但 OS 只捕获到 `(java)`，`process_release_proven=false`，suite 在 E1 停止，verdict `process_release_not_proven`，`p1_hard_gate_passed=false`。未 retry，未跑 E2–E12，未设 `integration_verified`。Canonical JAR 仍拒绝 `portal` DrawBlock；E12 fixture 只用于该授权校准。
