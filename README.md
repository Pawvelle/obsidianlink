...
唯一 active engineering task 是 `P1-REAL-MINERL-ENVIRONMENT-VALIDATION`。P1 validation runtime 与 E0 `reset_close` 离线实现已存在，但 E0 只是 `unit_verified`；真实 MineRL E0 尚未执行，E1–E12 尚未实现或尚未运行。没有启动真实 MineRL/Minecraft，没有运行 Gradle，没有调用付费模型 API。P1 Hard Gate 未通过，P2 不得开始。E10 原版水熔岩生成黑曜石、E11 portal activation、E12 dimension transition 均未获得 `integration_verified`。

下一步是在本 E0 实现审阅之后进行 P1 observation validation（E1–E3）。每次真实 MineRL 运行仍需单独授权。P1 的 hard gate 是稳定重复成功、`truth_missing=0`、无人工干预；建议至少 20 个 fresh episodes，最终次数在实验冻结阶段确定。
