...
唯一 active engineering task 是 `P1-REAL-MINERL-ENVIRONMENT-VALIDATION`。E0/E1 offline runtime 为 `unit_verified`。已审查一次成功的真实 E0 lifecycle run 和一次成功的真实 E1 RGB run（360×640×3 uint8）；E0 与 E1 都不是 `integration_verified`（无自动 promotion，P1 Hard Gate 未通过，`process_release_proven=false`）。E2–E12 尚未实现或尚未运行。没有运行 Gradle，没有调用付费模型 API。P1 Hard Gate 未通过，P2 不得开始。E10 原版水熔岩生成黑曜石、E11 portal activation、E12 dimension transition 均未获得 `integration_verified`。

下一步是 **Implement P1 E2 inventory observation**，不是现在执行 E2–E3 live run。每次真实 MineRL 运行仍需单独授权。P1 的 hard gate 是稳定重复成功、`truth_missing=0`、无人工干预；建议至少 20 个 fresh episodes，最终次数在实验冻结阶段确定。
