...
下一步不是自动重跑 E8，也不是开始 E9，也不是重跑 E6 或 E7。必须先审查 E8 live failure（`p1-e8-live-001`，`reset_failed` / `minecraft_native_crash`）。新的真实 E8 Minecraft 运行需要单独授权。P1 的 hard gate 是稳定重复成功、`truth_missing=0`、无人工干预；建议至少 20 个 fresh episodes，最终次数在实验冻结阶段确定。
