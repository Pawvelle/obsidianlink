...
唯一 active engineering task 是 `P1-REAL-MINERL-ENVIRONMENT-VALIDATION`。E0–E9 offline runtimes / adapters 为 `unit_verified`，且各有 reviewed real success；历史 E5/E8/E9 reset failures 保留为 `liblwjgl_stb` / Sound engine / `STBVorbis` infrastructure evidence，不能算 capability failure。Startup reliability audio mitigation 已部署，post-mitigation fresh-process validation 为 20/20 observed success（`max_reset_attempts=1`，native crash / Malmo EOF / timeout / cleanup failure 均为 0）；有限样本不证明绝对可靠。Attempt-014 历史根因仍未证明，post-mitigation 未复现。E0–E12 均不是 `integration_verified`。Object-level `process_release_proven` 仍为 false。E10 contract/offline runtime/MineRL bridge 为 `unit_verified`，controlled Mission geometry 已验证，一次真实 conversion (`p1-e10-live-001`) 为 `obsidian_conversion_ok`。E11 completion barrier 保留正常 vanilla client→server interaction，在 `ServerPlayNetHandler.processTryUseItemOnBlock` 完成后才允许 evaluator after-truth。真实 E11 (`p1-e11-completion-barrier-20260817-004`) 使用一个 fresh process、一次 reset、一次 `use_item(flint_and_steel)`、零 retry；before 为 14/14 obsidian、6/6 air、portal=0、`truth_missing_count=0`，after 为 portal=6/6、`truth_missing_count=0`，outcome `portal_activation_ok`。E11 real reviewed success 为 YES。E12 authorized fixture JAR（canonical + `e12-drawing-decorator-portal.patch`，不进入 `CANONICAL_PATCHES`）已用于一次真实 run `p1-e12-dimension-transition-20260817-001`：one fresh process、one reset、one `move(forward=1)`、zero retry；evaluator-only before=`minecraft:overworld`，after=`minecraft:the_nether`，`truth_missing_count=0`，outcome `dimension_transition_ok`。E12 real reviewed success 为 YES，仍不是 `integration_verified`。一次 E12 成功不是离开 P1 的稳定性 Hard Gate。P2 Benchmark Kernel v2 已 offline FROZEN；D1–D6 / L1–L4 / Multi-Agent 仍不得开始。

P2 Roadmap 在 step 1（minimal Benchmark Kernel freeze）已部分开始（commit `a5131dc`），P2 Benchmark Kernel v2 现已 offline FROZEN（freeze audit 2026-08-18，committed kernel `77c6c4e`）。该步骤不依赖 P1 hard gate，没有 LLM、没有真实 MineRL/Minecraft、没有 Gradle、没有付费 API，也没有把任何 E0–E12 标为 `integration_verified`。冻结内容包括 `BenchmarkRunRecord` 聚合（schema `p2.benchmark.run_record.v1`）、`run_benchmark` 单函数驱动（agent-visible / evaluator-only channel 隔离）、`write_run_record` / `load_run_record` JSON round-trip。完整 P2 推进（D1–D6、L1–L4、Multi-Agent）仍以 P1 重复性要求为准。P3 未开始。

当前完整 P1 suite 已获得一次真实完整成功运行：`p1-e0-e12-suite-20260817-004`，15/15，verdict `hard_gate_success`，该次 `p1_hard_gate_passed=true`，仍不设 `integration_verified`。Stability campaign `p1-stability-20260817` 在同一冻结版本上新跑 20 次 suite，结果 0/20 完整成功，20/20 在 E0 `reset_failed`（`MineRL reset failed after 1 attempts: a bytes-like object is required, not 'NoneType'`），未到达 E1–E12。历史失败证据保留：`p1-e0-e12-suite-20260817-001` 在 E1 因 OS `process_release_not_proven` 停止；第二次授权 live pilot `p1-e0-e12-suite-20260817-002` 跑到 E4：E0–E3 成功且 process-release 已证明；E4 `camera_ok` 但记录缺少 `truth_missing_count`，verdict `truth_missing`；第三次授权 live pilot `p1-e0-e12-suite-20260817-003` 跑到 E8：E0–E7（含 E7 water/lava）成功且 process-release 已证明；E8 `truth_block_unknown`（`tested_action_count=0`，`truth_missing_count` 为空），verdict `truth_missing`。授权 targeted E8 diagnostic `e8-live-20260817-001` 得到 `block_truth_ok`，Pilot #4 的 E8 也是 `block_truth_ok`。仓库尚未冻结 repeatability 数值阈值。Canonical JAR 仍拒绝 `portal` DrawBlock；E12 fixture 只用于该授权校准。
