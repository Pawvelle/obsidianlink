...
唯一 active engineering task 是 `P1-REAL-MINERL-ENVIRONMENT-VALIDATION`。E0–E5 offline runtime 为 `unit_verified`，并各有已审查的真实成功证据（E5 第二次为首次 reviewed movement success）。E6 contract / offline runtime / MineRL adapter 为 `unit_verified`，并有一次已审查的真实 `placement_ok`（`p1-e6-live-001`）。E7 contract / offline runtime / MineRL adapter 为 `unit_verified`；WATER 有一次已审查的真实 `bucket_ok`（`p1-e7-water-live-001`）；LAVA 有一次已审查的真实 `bucket_ok`（`p1-e7-lava-live-001`）；E7 不是 `integration_verified`。E8 contract / ServerTruthSnapshot block portion / generalized target-region block truth / MineRL adapter 为 `unit_verified` / offline verified；fluid portion 仍属 E9；一次真实 attempt `p1-e8-live-001` 在 reset 因 JVM SIGSEGV / Malmo EOF 失败，不是 block-truth 能力失败；E8 reviewed real success: NO，不是 `integration_verified`。E0–E8 都不是 `integration_verified`（无自动 promotion，P1 Hard Gate 未通过，`process_release_proven=false`）。E9–E12 尚未开始。没有运行 Gradle，没有调用付费模型 API。P1 Hard Gate 未通过，P2 不得开始。E10 原版水熔岩生成黑曜石、E11 portal activation、E12 dimension transition 均未获得 `integration_verified`。

E8 live failure review 已完成：`p1-e8-live-001` 与 E5 attempt #1 为 SAME_FINGERPRINT 的 LWJGL Sound engine native crash，不是 E8 block-truth 能力失败。第二次独立 E8 live attempt 可能合适，但需要新的单次授权。不要自动重跑，不要开始 E9，不要重跑 E6 或 E7。P1 的 hard gate 是稳定重复成功、`truth_missing=0`、无人工干预；建议至少 20 个 fresh episodes，最终次数在实验冻结阶段确定。
