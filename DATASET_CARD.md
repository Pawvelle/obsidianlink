...
R6-COMPLETE-PORTAL-FRAME — CONTRACT FREEZE 已在 [catalog](benchmark/catalog/tasks.json) 中新增 3 个 Benchmark 任务条目（Casting-S-C3 / C4 / C5 / fixed），`implementation_status="contract_only"`，没有真实 episode 数据。R6-C3-FRAME-EVALUATOR 子阶段在 FakeBackend 上完成了 C3 frame evaluator + task-origin / truth-grid 坐标锚定 + 独立 truth 注入路径（`obsidianlink/evaluation/casting_frame_evaluator.py` + `obsidianlink/env/fake.py` 的 `set_frame_evaluation_state` / `get_frame_evaluation_state` / `clear_frame_evaluation_state`），但**没有**任何真实 MineRL 端到端 episode 证据；任何对 C3 / C4 / C5 的统计与 leaderboard 数据仍必须在 evaluator / driver 真实离线验证（且 C3 / C4 / C5 三层全链验证）后、并经用户单独授权 MineRL / Gradle / 模型 API 后，才能进入正式结果。Ruined、Adaptive、Multi-Agent、统一未来字段和正式 train/dev/test 发布都只是规划，不能声称已有数据支持。
