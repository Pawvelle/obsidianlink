...
当前只能声称 v2 architecture/scope frozen、legacy infrastructure preserved、P1 E0–E9 runtime 离线 `unit_verified`，以及已审查的真实 E0 lifecycle、E1 RGB（360×640×3 uint8）、E2 inventory、E3 selected-item、E4 camera、E5 movement、E6 block-placement、E7 WATER/LAVA bucket-usage 与 E8 block-truth success evidence。E8 已实现 evaluator-only ServerTruthSnapshot 的 block portion 与 generalized target-region block truth。一次真实 attempt `p1-e8-live-001` 在 reset 因 JVM SIGSEGV / Malmo EOF 失败；已审查为与 E5 attempt #1 SAME_FINGERPRINT 的 LWJGL Sound engine infrastructure crash。第二次独立 attempt `p1-e8-live-002` 为已审查的真实 `block_truth_ok`；E8 reviewed real success: YES，仍不是 `integration_verified`。E9 已实现 generalized evaluator-only fluid truth（含 source/flowing 区分）与 MineRL adapter/live bridge，离线 `unit_verified`。一次 WATER 真实 attempt `p1-e9-water-live-001` 在 reset 因 JVM SIGSEGV / Malmo EOF 失败；已审查为与 E5 attempt #1 / E8 attempt #1 SAME_FINGERPRINT 的 LWJGL Sound engine infrastructure crash；没有 fluid snapshots，不是 fluid-truth 能力失败。第二次独立 WATER attempt `p1-e9-water-live-002` 为已审查的真实 `fluid_truth_ok`：after 目标格 `observed_block=water` / `flow_state=source`，`truth_missing_count=0`。一次独立 LAVA attempt `p1-e9-lava-live-001` 为已审查的真实 `fluid_truth_ok`：after 目标格 `observed_block=lava` / `flow_state=source`，`truth_missing_count=0`。E9 WATER reviewed real success: YES。E9 LAVA reviewed real success: YES。E9 real calibration coverage: WATER + LAVA complete。E9 reviewed real success: YES。E9 不是 `integration_verified`。E0–E9 都不是 `integration_verified`。E10–E12 尚未实现、没有 end-to-end task implementation、没有 Multi-Agent gameplay、没有 formal dataset 或 benchmark evaluation。P1 Hard Gate 未通过，P2 不得开始。
